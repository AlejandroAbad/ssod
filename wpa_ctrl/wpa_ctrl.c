...
try_again:
	...
	if (strncmp(ctrl_path, "@abstract:", 10) == 0)
	{
		...
		strlcpy(ctrl->dest.sun_path + 1, ctrl_path + 10, sizeof (ctrl->dest.sun_path) - 1);
	}
	else
	{
	...
	setsockopt(ctrl->s, SOL_SOCKET, SO_RCVTIMEO, (char *) &tv, sizeof (struct timeval));
